...
A 15-strategy algorithmic paper-trading platform spanning equities, options, and crypto, running 24/7 on a single AWS EC2 instance. Every strategy runs as an isolated, supervised Linux service with its own brokerage account, risk limits, alerting channel, and log rotation — coordinated by a shared risk engine and a nightly analytics pipeline.
...
flowchart TD
    subgraph EC2["AWS EC2 t3.medium — Ubuntu, systemd"]
        subgraph BOTS["15 strategy bots (isolated services)"]
            MR[Mean Reversion]
            ...
            CT[Crypto Trend BTC/ETH<br/>24/7]
            MORE[...7 more]
        end
...
| Component | Role |
|---|---|
| **15 strategy bots** | Each an independent Python service with its own $100k paper account — 13 equity/options strategies plus BTC and ETH trend bots trading 24/7. Seven representative bots are in this repo (see below). |
...
Fifteen bots run in production, but several are controlled variants of each other (that's the experiment). The seven here each demonstrate a distinct engineering pattern:
...
| [`bots/crypto-trend`](bots/crypto-trend) | 24/7 asset class: stateless position tracking (the broker is the source of truth), daily decision cadence, no market-hours logic. Deployed only after a spread audit and a pre-registered 5-year backtest (see [`tools/backtest`](tools/backtest)) |
...
**Backtest as a gate, not an oracle.** New strategies are backtested with pre-registered rules and conservative costs before deployment — the crypto bots' sizing and kill-lines came directly from measured historical drawdowns. Backtests of already-live strategies are informational only: see [`tools/backtest/RESULTS.md`](tools/backtest/RESULTS.md) for the full results, most of which are unflattering, and the reasoning for not acting on them.
...
```
bots/                  seven representative strategy services
...
tools/backtest/        pre-registered backtests + honest results writeup
